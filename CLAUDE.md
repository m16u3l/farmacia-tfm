...
- Schema source of truth is `db/schema.sql` (tables: `users`, `products`, `suppliers`, `inventory`, `employees`, `orders`/`order_items`, `sells`/`sell_items`, `configuracion`, `audit_log`). Incremental changes go in `db/migrations/`.
