  ...
  try {
    ...
    try {
      ...
      const result = await client.query(
        ...
        [name, description, possible_uses ?? null, additional_info ?? null, laboratory ?? null, active_ingredient ?? null, concentration ?? null, health_registry ?? null, category, type, dosage_form, unit, dosage_instructions ?? null, normalizedBarcode, sale_control ?? "libre", low_stock_threshold ?? null, status, session?.userId ?? null]
      );
